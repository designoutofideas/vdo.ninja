...
I'm concerned at times that less experienced users will be deploying the code without really understanding why or properly how to.  There are few cases a person needs to deploy any code. Those reasons are:  wanting custom branding; contributing as a developer to the codebase; deploying a private TURN server; or running Voices without internet on a private LAN.
...
There's also the Github-hosted version, which mirrors the master branch of the code repo. https://steveseguin.github.io/vdo.ninja/  You can use this hosted version or fork Voices and host it yourself in the same way via Github Pages; it's free and can be done within minutes. I provide a video guide on how to do this here, https://www.youtube.com/watch?v=uYLKkX2_flY, and it's what I recommend for most users looking to deploy their own version of Voices.

For those wanting a private TURN server setup, you can load the settings for those via the URL parameters. If infrequently needing a private TURN, this is a great solution.  You can also use URL forwarding services to load up a customized link to Voices, with URL parameters already included, such as https://invite.mypersonaldomain.com , which might secretly resolve to https://vdo.ninja/?room=myRoom&hash=3423&label or such.

Voices also supports IFRAMES, so you can embed Voices into your website and customize it via both URL parameters, but also via the IFRAME API.  You can insert custom CSS styles with this method, giving Voices quite a bit of flare.

See more on IFRAMES here: https://docs.vdo.ninja/guides/iframe-api-documentation

Understanding clearly why you need to deploy any code or server is important. Maintaining updated deployed code can be quite hard, as Voices updates frequently, so there are good reasons to consider an IFRAME approach instead. Feature requests there are welcomed.
...
For a very simple method on how to deploy Voices, there's a detailed video guide here: https://www.youtube.com/watch?v=uYLKkX2_flY 
...
Please note, Voices REQUIRES a domain name and SSL, unless you modify all browsers being used to support otherwise. (More on this in the [Internet-free section](#internet-free-deployments) below)  As a result, getting Voices working can be quite challenging, as setting up domain names and SSL can be tricky for some. 
...
If you need to download the code for Voices, the basic idea is something like this:
...
To keep things easy, Steve generally recommends using Cloudflare to provide caching and SSL, but you can google `Certbot` for another free SSL option. The below NGINX config assumes you are using Cloudflare's flexible SSL option, which is the simpliest way to get started.  You'll need to also add the Voices code to the /var/www/html/vdo.ninja folder (or whatever you set it to) and modify the port/SSL/domain-name settings as needed.
...
Once you have your TURN server setup, you can update the index.html of the Voices code (near the bottom) with your TURN/STUN settings.
...
A new deployment of Voices should work without any changes to the index.html file, although you'll want to change it to support your own TURN server and perhaps branding. The Voices code needs to be constantly kept up to date though; this is the reality of deploying Voices -- you should update it every few months at the very least. As a result, keep this in mind when making changes to the Voices source code, as heavy custom changes will make updating harder to do. The fewer the changes the better.
...
Note, if doing things yourself, since you will be using a private handshake server, don't forget that you'll need to specify that in the Voices index.html file.
...
Internet-free deployments will also need to deal with private SSL certificates and any DNS secure context issues that may arise. Voices relies on SSL for security, but without Internet, you'll need to create and use a private ceriticate that get added to your system's trusted certificate key chain.
...
If you can't figure out how to do private SSL issuance, these SSL restrictions can be somewhat disabled at the browser's command-line or for localhost via the Chrome://flags settings. Voices may complain about the lack of security if you take this approach though, but you can edit out those lines of code which trigger those warnings as needed. You may still need to issue an SSL certificate, self-signed or what not, but with these flags enabled it doesn't at least need to be valid.
