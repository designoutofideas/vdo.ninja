...
## Voices - iFrame API documentation (depreciated; see above)

Voices (VDON) is offers here a simple and free solution to quickly enable real-time video streaming in their websites. VDON wishes to make live video streaming development accessible to any developer, even novices, yet still remain flexible and powerful.

While Voices does offer source-code to customize the application and UI at a low level, this isn't for beginners and it is rather hard to maintain. As well, due to the complexity of video streaming in the web, typical approaches for offering API access isn't quite feasible either.

The solution decided on isn't an SDK framework, but rather the use of embeddable _IFrames_ and a corresponding bi-directional iframe API. An [iframe](https://www.w3schools.com/tags/tag_iframe.ASP) allows us to embed a webpage inside a webpage, including Voices into your own website.
...
You can also make an Voices without Javascript, using just HTML, like
...
An example of how this API is used by Voices is with its Internet Speedtest, which has two Voices IFrames on a single page. One iframe feeds video to the other iframe, and the speed at which it does this is a measure of the system's performance. Detailed stats of the connection are made available to the parent window, which displays the results.
...
A sandbox of options is available at this page, too: https://vdo.ninja/iframe You can enter an Voices URL in the input box to start using it. For developers, viewing the source of that page will reveal examples of how all the available functions work, along with a way to test and play with each of them. You can also see here for the source-code on GitHub: https://github.com/steveseguin/vdoninja/blob/master/iframe.html

One thing to note about this iframe API is that it is a mix of URL parameters given to the iframe _src_ URL, but also the postMessage and addEventListener methods of the browser. The later is used to dynamically control Voices, while the former is used to initiate the instance to a desired state.
...
This Voices API is developed and expanded based on user feedback and requests. It is by no means complete.

Please feel free to follow me in the Voices Discord channel (discord.vdo.ninja) where I post news about updates and listen to requests. The upcoming version of Voices is also often hosted at https://vdo.ninja/beta, where you can explore new features and help crush any unexpected bugs.
