# Understanding the Voices IFRAME API: Detecting User Joins and Disconnects

The Voices IFRAME API allows websites to embed and interact with Voices streams. One of the most useful features is the ability to detect when users join or disconnect from your stream through event messaging. This guide will explain how to implement this functionality in your own projects.

## How the IFRAME API Works

Voices's IFRAME API uses the browser's `postMessage` API to communicate between your parent website and the embedded Voices iframe. This allows you to:

1. Send commands to control the Voices instance
2. Receive events and data from the Voices instance

## Setting Up the Basic Structure

First, you need to create an iframe that loads Voices:
...
// Set the source URL (your Voices room)
...
eventer(messageEvent, function (e) {
    // Make sure the message is from our Voices iframe
...
1. **Always check the source**: Make sure messages are coming from your Voices iframe.
...
By implementing these techniques, you can build sophisticated applications that respond to users joining and leaving your Voices sessions, creating more interactive and responsive experiences.

# Voices IFRAME API - Complete Inbound Control Reference

This document provides a comprehensive list of all inbound remote control calls available through the Voices IFRAME API. These commands allow you to control a Voices instance embedded in an iframe from your parent webpage.
...
To send commands to the Voices iframe:
